...
variable is \code{group} and the
explanatory variables are \code{insulin}, \code{glucose} and \code{sspg}. The code
\begin{Scode}
R> x <- diabetes[, c("insulin", "glucose", "sspg")]
...
\begin{Scode}
R> LdaClassic(group~insulin+glucose+sspg, data = diabetes)
\end{Scode}
...
the context of robust linear discriminant analysis.

We start with bringing the data set \code{diabetes} into the workspace by typing
<<lda-prelim>>=
data("diabetes")
@
...
\begin{Scode}
R> rlda <- Linda(group~insulin+glucose+sspg, data = diabetes, method = "fsa")
\end{Scode}
